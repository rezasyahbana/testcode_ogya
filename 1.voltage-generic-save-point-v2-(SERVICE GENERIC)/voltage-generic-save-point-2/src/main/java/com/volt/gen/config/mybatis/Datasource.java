...
public class Datasource {
    ...
    static void addDatasource(String dsName, Map<String, String> dsConf,
        ...
        boolean autoCommit =
                dsConf.containsKey(AUTO_COMMIT) ? Boolean.valueOf(dsConf.get(AUTO_COMMIT)) : true;
        Integer maximumPoolSize =
                dsConf.containsKey(MAX_POOL_SIZE) ? Integer.valueOf(dsConf.get(MAX_POOL_SIZE))
                        : 1000;
        Integer minimumIdle =
                dsConf.containsKey(MIN_IDLE) ? Integer.valueOf(dsConf.get(MIN_IDLE)) : 1;
        ...
        Integer connectionTimeout = dsConf.containsKey(CONNECTION_TIMEOUT)
                ? Integer.valueOf(dsConf.get(CONNECTION_TIMEOUT))
                : 2000;
        Integer idleTimeout =
                dsConf.containsKey(IDLE_TIMEOUT) ? Integer.valueOf(dsConf.get(IDLE_TIMEOUT))
                        : 900000;
        Integer maxLifetime =
                dsConf.containsKey(MAX_LIFETIME) ? Integer.valueOf(dsConf.get(MAX_LIFETIME))
                        : 1800000;
        ...
        try {
            ...
            if (!applicationContext.containsBean(poolName)) {
                ((GenericApplicationContext) applicationContext).registerBean(poolName,
                        HikariDataSource.class, dataSource);
            }
    ...
    static void updateDatasource(String dsName, HikariDataSource hikariDS,
